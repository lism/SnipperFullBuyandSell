from Liquidity import PanckaeswapNewListing
...
configFilePath = "D:\Development\SnipperFullBot\Config.json"
...
def updateTitle():
    ...
    ctypes.windll.kernel32.SetConsoleTitleW(
        "BSCTokenSniper | Tokens Detected: " + str(numTokensDetected) + " | Tokens Bought: " + str(
...
def Buy(tokenAddress, tokenSymbol):
    ...
    if (tokenAddress != None):
        ...
        try:
            ...
            isSell=False
            print("No Worry We are Fetching the Another Token for You!!")  # line break: move onto scanning for next token
        ...
        if (jsonEventContents['args'][
            ...
            if (enableMiniAudit == True):  # enable mini audit feature: quickly scans token for potential features that make it a scam / honeypot / rugpull etc
                ...
                if (str(tokenContractCode['result'][0][
                    ...
                    Sell(bsc, web3, pancakeSwapRouterAddress, walletAddress, tokenAddress, private_key)

                    liquidty=PanckaeswapNewListing().run(tokenAddress)
                    if(observeOnly == "True" and liquidty > 0.01 ):
                         Buy(tokenAddress, tokenSymbol)
                    if (isSell == True):
                        Sell(bsc, web3, pancakeSwapRouterAddress, walletAddress,tokenAddress, private_key)

                    else:
                        print('Liquidity is 0 for '+ tokenAddress )
